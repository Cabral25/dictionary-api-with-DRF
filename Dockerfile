...
# Se /app não existir, o docker cria.
...
CMD ["sh", "-c", "python manage.py migrate && python manage.py create_admin && gunicorn dictionary_api_with_DRF.wsgi:application --bind 0.0.0.0:$PORT"]